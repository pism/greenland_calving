...
class Makefile(object):
    ...
    def add(self, inputs, outputs, action):
    ...
    def get_dates(self, targets):
        ...
        for target in targets:
            ...
            if target in self.rules:
                ...
                input_dates = self.get_dates(rule.inputs, dates)  # [(date,max_sub), ...]
...
class tiffs_to_netcdfs(object):
    def __init__(self, makefile, idir, odir, parameter, reftime='2008-01-01', blacklist=set(), max_files=10000000, filter_attrs=dict()):
        ...
        inputs = list()
        outputs = list()
        for ix,pf in enumerate(pfiles_tif):
...
def main():
    makefile = Makefile()
    tiffs_to_netcdf(makefile, 'data', 'outputs', 'vx')
    print(makefile.rules)
#
#
#class tiffs_to_netcdfs(object):
#    def __init__(self, idir, odir, reftime='2008-01-01', blacklist=[], max_files=None):
#        autoinit()
#
#        # Get list of .tif files
#        self.pfiles_tif = iopfile.listdir(idir, nsidc.parse_0481,
...
#
#            # Don't iterate over blacklisted items
#            if pf_tif.leaf in self.blacklist:
#                continue
#
#            # Cut it off early
#            if ix >= max_files:
...
#        # Set list of files
#        self.files = (
#            [pf.format(ext='.tif') for pf in self.pfiles_nc],    # Inputs
#            [pf.format(ext='.nc') for pf in self.pfiles_nc])    # Outputs
#
#    def __call__(self):
#        for ipath,opath in zip(*self.files):
#            
#            nsidc.tiff_to_netcdf(pf_tif, odir, reftime=self.reftime)
#
#def tif_to_nc(idir, odir):
#    # Get list of .tif files
