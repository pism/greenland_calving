...
# -------------------------------------------------------
def d_dy(data, dmap,dy, smap,  rows,cols,vals,bb, factor=1.0, rowoffset=0, coloffset=0):
...
# ----------------------------------------------------------------
def d_dx(data, dmap,dx, smap,  rows,cols,vals,bb,
    factor=1.0, rowoffset=0, coloffset=0):

    d_dy(np.transpose(data), np.transpose(dmap),dx, smap.transpose(),
        rows,cols,vals,bb,
        factor=factor, rowoffset=rowoffset, coloffset=coloffset)

# ----------------------------------------------------------------
def div_matrix(vvel2,uvel2, dmap, dyx, smap, rows,cols,vals,bb,
    ...
    """
    d_dy(vvel2, dmap, dyx[0], smap, rows,cols,vals,bb,
        factor=factor, rowoffset=rowoffset)
    d_dx(uvel2, dmap, dyx[1], smap, rows,cols,vals,bb,
        factor=factor, rowoffset=rowoffset, coloffset=len(smap))


def curl_matrix(vvel2,uvel2, dmap, dyx, smap, rows,cols,vals,bb,
    factor=1.0, rowoffset=0):
    """Generates a matrix to compute divergence of a vector field.
    (v and u are stacked)
...
def main():
    ...
    # Make an "unused" border as a sentinel
    dmap2[0,:] = D_UNUSED
    dmap2[-1,:] = D_UNUSED
    dmap2[:,0] = D_UNUSED
    dmap2[:,-1] = D_UNUSED
